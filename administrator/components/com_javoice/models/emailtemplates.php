...
class javoiceModelemailtemplates extends JAVBModel
{
   var $_data;
    var $_table;
    var $_pagination;
	...
	*/
    function getItems($filter_lang='', $type=false){
    	...
    	if($search){
    		$where = " AND LOWER(name) like ".$db->Quote( '%'.$db->escape( $search, true ).'%', false )
    				. " or LOWER(subject) like ".$db->Quote( '%'.$db->escape( $search, true ).'%', false );
    	}
    	if ( $filter_state )
		{
			if ( $filter_state == 'P' ) {
	...
	*/
	function remove(){
	    ...
	    if($n){
	        $query = "DELETE FROM #__jav_email_templates WHERE id = ". implode(" OR id = ", $cid);
	        $db = JFactory::getDBO();
	        $db->setQuery($query);
	        if($db->query()){
	            return $n;
	...
	*/
	function store(){
		...
		$post ['emailcontent'] = JRequest::getVar ( 'emailcontent', '', 'post', 'string', JREQUEST_ALLOWRAW );
		...
		if (!$row->store())
		{
			JError::raiseWarning(1, JText::_('ERROR_OCCURRED_DATA_NOT_SAVED'));
			return false;			
		}
	...
	function dopublish($publish){
		...
		$query = "UPDATE #__jav_email_templates"
	...
	function getItemsbyWhere($cid=''){
		$db = JFactory::getDBO();
		$sql = "select * from #__jav_email_templates where id in ($cid) ";
		$db->setQuery($sql);
	...
	function getItembyWhere($where=''){
		$db = JFactory::getDBO();
		$sql = "select id from #__jav_email_templates where 1=1 $where";
		$db->setQuery($sql);
	...
	*/
	...
	function import($filecontent){
		...
		if(!$list){
			JError::raiseWarning(1, JText::_('IMPORT_FAILURE_FILE_IS_EMPTY_OR_DATA_IS_NOT_CORRECT_FORMAT'));
			return false;
		}
		...
		foreach ($list as $item){
			if(isset($item['name']) && $item['name']!='' && $filter_lang!=''){				
				$id = $this->getItembyWhere(" and language='{$filter_lang}' and name='{$item['name']}'");
		...
		while ($source!='') {
			...
			if ($start !== FALSE && $end !== FALSE) {
				...
				if( $end_first_tag!== FALSE ){
					...
					$arr_field = array('title', 'subject', 'emailcontent', 'EmailFromAddress', 'EmailFromName');					
					$arr_pos = array();
