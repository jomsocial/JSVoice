			...
			if ($user_ids) {
				...
				if ($count > 0) {
					...
					for($i = 0; $i < $n; $i ++) {
						...
						$filters['{EMAIL_PREFERENCE_LINK}'] = "<a href=\"http://giapnd.javoice.com/index.php?option=com_javoice&view=users&uid=$user->id&tab=2\">".JText::_("TURN_OFF_OR_EDIT_YOUR_EMAIL_NOTIFICATIONS").".<a>";
...
function changeLog($voicetype_id, $item_id, $model_items) {
	$model_voicetype = JModel::getInstance ( 'voicetypes', 'javoiceModel' );
