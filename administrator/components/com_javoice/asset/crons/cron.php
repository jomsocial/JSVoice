...
function countItemInForum(){
	...
	if($count){
		for ($i=0;$i<$count;$i++){
			$item = $items[$i];
			if(!$status_spam_ids[$item->voice_types_id])
			...
			if($status_spam_ids[$item->voice_types_id])$query.=" AND voice_type_status_id NOT IN ({$status_spam_ids[$item->voice_types_id]}) ";
			$db->setQuery($query);
