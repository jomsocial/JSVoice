<?php
      ...
      class jacapcha
      {
       ...
       // use P=point,L=line,C=circle,E=elipse,CF=fillcircle,EF=fillelipse;T=text
        ...
        var $im;
        var $ttf_file = "components/com_javoice/helpers/jacaptcha/font.ttf";
        var $bgimg;
        var $text_length = 5;
        ...
        function buildImage($type = "addnew"){
		    ...
		    if (!$this->draw_lines_over_text && $this->draw_lines) $this->addLines();
		    //$this->addPatern();
		    $this->addWord();
		    if ($this->arc_linethrough == true) $this->addarcLines();
		    if ($this->draw_lines_over_text && $this->draw_lines) $this->addLines();
		    $this->outputimage();
	    }
        ...
        function setBackground(){
        	$imagefile = @getimagesize($this->bgimg);
		 	...
		 	$type = $imagefile[2];
		    switch($type) {
		      case 1:  $backimg = @imagecreatefromgif($this->bgimg); break;
		      case 2:  $backimg = @imagecreatefromjpeg($this->bgimg); break;
		      case 3:  $backimg = @imagecreatefrompng($this->bgimg); break;
		      case 15: $backimg = @imagecreatefromwbmp($this->bgimg); break;
		      case 16: $backimg = @imagecreatefromxbm($this->bgimg); break;
		      default: return;
		    }
		    if(!$backimg) return;
         ...
         /* Add the CAPTCHA text over the image */
		  function addWord()
		  {
		   ...
		   function addPatern(){
		    ...
		    if($PatNum == ''){
		      if(strpos($PatType,'c') !== false) $PatNum = 50;
		      elseif(strpos($PatType,'e') !== false) $PatNum = 50;
		      elseif($PatType == 't') $PatNum = 500;
		      elseif($PatType == 'p') $PatNum = 2;
		      elseif($PatType == 'l') $PatNum = 10;
		      else $PatNum = 10;
		    }
